  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError((json as { error?: string }).error ?? t("editForm.saveFailed"))
        return
      }
      router.refresh()
      router.push(`/crm/salespersons/${salesperson.id}`)
    } catch {
      setError(t("editForm.saveFailed"))
    } finally {
      setLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" disabled={loading} className="bg-[var(--crm-brand)] text-white hover:bg-[var(--crm-brand-dark)]">
          {loading ? t("editForm.saving") : t("editForm.save")}
        </Button>
        <Button asChild variant="outline">
          <Link href={`/crm/salespersons/${salesperson.id}`}>{t("editForm.cancel")}</Link>
        </Button>
      </div>
    </form>
  )
