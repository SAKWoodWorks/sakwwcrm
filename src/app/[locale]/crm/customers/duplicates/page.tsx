...
export default async function CustomerDuplicatesPage() {
  const t = await getTranslations("CustomerDuplicates")
    ...
    <div className="crm-page max-w-5xl">
      <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <Button asChild variant="outline">
          <Link href="/crm/customers">{t("back")}</Link>
        </Button>
      </div>

      <Card className="mb-4 rounded-lg border-[var(--crm-line)] bg-white shadow-[var(--crm-shadow)]">
        <CardContent className="p-4">
          <h2 className="text-base font-semibold text-[var(--crm-ink)]">{t("manual.title")}</h2>
          <p className="mt-1 text-sm text-[var(--crm-muted)]">
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-2">
        <SummaryCard title={t("sections.name")} groupCount={nameSummary.groups} customerCount={nameSummary.customers} localeTag={localeTag} t={t} />
        <SummaryCard title={t("sections.taxId")} groupCount={taxIdSummary.groups} customerCount={taxIdSummary.customers} localeTag={localeTag} t={t} />
      </div>

      <DuplicateSection
        title={t("sections.name")}
        emptyText={t("empty.name")}
        groups={nameGroups}
        localeTag={localeTag}
        t={t}
      />
      ...
      <div className="mt-6">
        <DuplicateSection
          title={t("sections.taxId")}
          emptyText={t("empty.taxId")}
          groups={taxIdGroups}
          localeTag={localeTag}
          t={t}
        />
    ...
    <Card className="rounded-lg border-[var(--crm-line)] bg-white shadow-[var(--crm-shadow)]">
      <CardContent className="p-4">
        ...
        <p className="mt-1 text-2xl font-semibold text-[var(--crm-ink)]">
          {t("summary.customers", { count: customerCount.toLocaleString(localeTag) })}
        </p>
        <p className="mt-1 text-xs text-[var(--crm-muted)]">{t("summary.groups", { count: groupCount.toLocaleString(localeTag) })}</p>
      </CardContent>
    </Card>
  )
}
                ...
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[var(--crm-muted)]">{t("records", { count: Number(group.customer_count).toLocaleString(localeTag) })}</p>
                    </div>
                  </div>
                  ...
                  {group.customer_ids.map((id, index) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 md:text-right">
                        {t("documents", { count: Number(group.document_counts[index] ?? 0).toLocaleString(localeTag) })}
                      </div>
                      <Button asChild variant="outline" size="sm">
                        <Link href={`/crm/customers/${id}`}>{t("open", { id })}</Link>
                      </Button>
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-gray-500">
                          {t("mergeHint", { id, ids: formatIds(group.customer_ids.filter((otherId) => otherId !== id)) })}
                        </p>
                      </div>
                    </div>
                  ))}
