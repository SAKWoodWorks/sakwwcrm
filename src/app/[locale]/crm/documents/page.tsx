...
export default async function DocumentsPage({ searchParams }: Props) {
  ...
  const currentListHref = buildPageUrl(page)
  const documentDetailHref = (documentId: number) =>
    `/crm/documents/${documentId}?${new URLSearchParams({ returnTo: currentListHref }).toString()}`
      ...
      <div className="crm-mobile-list">
        {documents.map((d) => (
          <Card key={d.id} className="rounded-lg border-[var(--crm-line)] bg-white p-4 shadow-[var(--crm-shadow)]">
            ...
            <div className="mt-3">
              <p className="text-xs text-[var(--crm-muted)]">{t("table.customer")}</p>
              {d.customer ? (
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm font-medium">{formatSalesperson(d.salesperson?.name, t("unknownSalesperson"))}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-[var(--crm-muted)]">{t("table.amount")}</p>
                <p className="font-semibold tabular-nums">
                  {d.total != null
                    ? Number(d.total).toLocaleString(localeTag, { style: "currency", currency: "THB", minimumFractionDigits: 0 })
                    : "—"}
                </p>
              </div>
            </div>
            ...
            <TableRow>
              <TableHead className="px-4 py-3 text-gray-500">
                <Link href={sortUrl("docDate")} className="hover:text-gray-800">
                  {t("table.date")}{indicator("docDate")}
                </Link>
              </TableHead>
              ...
              <TableHead className="px-4 py-3 text-gray-500">{t("table.channel")}</TableHead>
              ...
              <TableHead className="px-4 py-3 text-right text-gray-500">
                <Link href={sortUrl("total")} className="hover:text-gray-800">
                  {t("table.amount")}{indicator("total")}
                </Link>
              </TableHead>
          ...
          <TableBody>
            {documents.map((d) => (
              <TableRow key={d.id} className="hover:bg-gray-50">
                ...
                <TableCell className="px-4 py-3">{formatSalesperson(d.salesperson?.name, t("unknownSalesperson"))}</TableCell>
                <TableCell className="px-4 py-3 text-right tabular-nums">
                  {d.total != null
                    ? Number(d.total).toLocaleString(localeTag, {
                        style: "currency",
                        currency: "THB",
